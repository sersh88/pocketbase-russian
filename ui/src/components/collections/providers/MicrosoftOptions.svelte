...
<div class="section-title">Эндпоинты Azure AD</div>
<Field class="form-field required" name="{key}.authURL" let:uniqueId>
    ...
    <div class="help-block">
        Пример: {`https://login.microsoftonline.com/YOUR_DIRECTORY_TENANT_ID/oauth2/v2.0/authorize`}
    </div>
</Field>
<Field class="form-field required" name="{key}.tokenURL" let:uniqueId>
    ...
    <div class="help-block">
        Пример: {`https://login.microsoftonline.com/YOUR_DIRECTORY_TENANT_ID/oauth2/v2.0/token`}
    </div>
</Field>
