...
{#if field.primaryKey}
    ...
    {#if record.collectionName == "_superusers" && record.id == $superuser.id}
        <span class="label label-warning">Ты</span>
    {/if}
...
{:else if CommonHelper.isEmpty(rawValue)}
    <span class="txt-hint">Н/Д</span>
{:else if field.type === "bool"}
    <span class="label" class:label-success={!!rawValue}>{rawValue ? "Да" : "Нет"}</span>
...
{:else if field.type === "url"}
    <a
        ...
        use:tooltip={"Открыть в новой вкладке"}
